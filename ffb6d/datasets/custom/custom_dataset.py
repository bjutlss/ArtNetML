...
class Dataset():
    ...
    def __init__(self, dataset_name, cls_type="vase", DEBUG=False):
        self.DEBUG = DEBUG
        self.config = Config(ds_name='custom', cls_type=cls_type)
        ...
        self.obj_dict = self.config.custom_obj_dict

        self.cls_type = cls_type
        self.cls_id = self.obj_dict[cls_type]
        print("cls_id in custom_dataset.py", self.cls_id)
        self.root = os.path.join(self.config.custom_root, 'datasets/custom/custom_preprocessed')
        ...
        if dataset_name == 'train':
            ...
            if len(self.rnd_lst) == 0:
                warning = "Warning: "
                warning += "Trainig without rendered data will hurt model performance \n"
            ...
            if len(self.fuse_lst) == 0:
                warning = "Warning: "
                warning += "Trainig without fused data will hurt model performance \n"
    ...
    def get_item(self, item_name):
        if "pkl" in item_name:
            ...
            rnd_typ = 'real'
            K = self.config.intrinsic_matrix["custom"]
        cam_scale = 1000.0
    ...
    def get_pose_gt_info(self, cld, labels, RT):
        ...
        for i, cls_id in enumerate([1]):
            ...
            ctr = self.bs_utils.get_ctr(self.cls_type, ds_type="custom")[:, None]
            ...
            kps = self.bs_utils.get_kps(
                self.cls_type, kp_type=kp_type, ds_type='custom'
            )
