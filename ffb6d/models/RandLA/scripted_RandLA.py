...
''' scripting & tracing '''
int_in = randrange(10)
int_out = randrange(10)

#traced_att_pooling = trace(Att_pooling(), int_in, int_out)
#print(traced_att_pooling.graph)

#trace_conv1d = trace(pt_utils.Conv1d(), torch.rand(10, 3, 50, 50))
#traced_building_block = trace(Building_block())

#traced_dilated= trace(Dilated_res_block())

from ffb6d.common import ConfigRandLA
config = ConfigRandLA
traced_model = script(Network(config=config)))
print(traced_model.graph)
#scripted_network = script(Network())
#torch.jit.save(scripted_network, 'compiled_ffb6d.pt')