...
from .ip_basic.ip_basic import depth_map_utils_ycb as depth_map_utils


intrinsic_matrix = {
    'custom': np.array([[572.4114, 0.,         325.2611],
                        [0.,        573.57043,  242.04899],
                        [0.,        0.,         1.]]),

    'custom':   np.array([[1594.7247, 0.,         951.2392],
                        [0.,        1594.7247,  722.79],
                        [0.,        0.,         1.]]),
...
class PoseTransformer(object):
    ...
    def __init__(self, class_type):
        ...
        custom_pth = 'datasets/custom/custom'
        custom_occ_pth = 'datasets/custom/OCCLUSION_custom'
        self.blender_model_path = os.path.join(custom_pth, '{}/{}.ply'.format(class_type, class_type))
        self.xyz_pattern = os.path.join(custom_occ_pth, 'models/{}/{}.xyz')
...
class Basic_Utils():
    ...
    def __init__(self, config):
        ...
        if config.dataset_name == "custom":
            self.xmap = np.array([[j for i in range(1440)] for j in range(1920)])
            self.ymap = np.array([[i for i in range(1440)] for j in range(1920)])
        else:
        ...
        self.custom_cls_ptsxyz_dict = {}
        self.custom_cls_ptsxyz_cuda_dict = {}
        self.custom_cls_kps_dict = {}
        self.custom_cls_ctr_dict = {}
        self.custom_cls_ptsxyz_dict = {}
        self.custom_cls_ptsxyz_cuda_dict = {}
        self.custom_cls_kps_dict = {}
        self.custom_cls_ctr_dict = {}
    ...
    def get_normal_map(self, nrm, choose):
        nrm_map = np.zeros((1920, 1440, 3), dtype=np.uint8)
        nrm = nrm[:, :3]
        ...
        nrm_map = nrm_map.reshape((1920, 1440, 3))
        return nrm_map

    def get_rgb_pts_map(self, pts, choose):
        pts_map = np.zeros((1920, 1440, 3), dtype=np.uint8)
        pts = pts[:, :3]
        ...
        pts_map = pts_map.reshape((1920, 1440, 3))
        return pts_map
    ...
    def get_pointxyz(
        ...
        if ds_type == "ycb":
            ...
            ptxyz_pth = os.path.join(
                'datasets/custom/custom_preprocessed/models',
                'obj_%02d.ply' % cls
            )
            ...
            self.custom_cls_ptsxyz_dict[cls] = pointxyz
            return pointxyz
        else:
            ptxyz_pth = os.path.join(
                'datasets/custom/custom_preprocessed/models',
    ...
    def get_pointxyz_cuda(
        ...
        if ds_type == "ycb":
            ...
            if cls in self.custom_cls_ptsxyz_cuda_dict.keys():
                return self.custom_cls_ptsxyz_cuda_dict[cls].clone()
            ptsxyz = self.get_pointxyz(cls, ds_type)
            ptsxyz_cu = torch.from_numpy(ptsxyz.astype(np.float32)).cuda()
            self.custom_cls_ptsxyz_cuda_dict[cls] = ptsxyz_cu
            return ptsxyz_cu.clone()
        else:
            if cls in self.custom_cls_ptsxyz_cuda_dict.keys():
                return self.custom_cls_ptsxyz_cuda_dict[cls].clone()
    ...
    def get_kps(
        ...
        if type(cls) is int:
            if ds_type == 'ycb':
                ...
                cls = self.config.custom_id2obj_dict[cls]
            else:
                cls = self.config.custom_id2obj_dict[cls]
        try:
        ...
        if ds_type == "ycb":
            ...
            if cls in self.custom_cls_kps_dict.keys():
                return self.custom_cls_kps_dict[cls].copy()
            if use_orbfps:
                kps_pth = self.config.kp_orbfps_ptn % (cls, self.config.n_keypoints)
            else:
                kps_pattern = os.path.join(
                    self.config.custom_kps_dir, "{}/{}.txt".format(cls, kp_type)
                )
                kps_pth = kps_pattern.format(cls)
            print("kps_pth in get_kps:", kps_pth)
            kps = np.loadtxt(kps_pth, dtype=np.float32)
            self.custom_cls_kps_dict[cls] = kps
        else:
            if cls in self.custom_cls_kps_dict.keys():
                return self.custom_cls_kps_dict[cls].copy()
            if use_orbfps:
    ...
    def get_ctr(self, cls, ds_type='ycb', ctr_pth=None):
        ...
        if ds_type == "ycb":
            ...
            if cls in self.custom_cls_ctr_dict.keys():
                return self.custom_cls_ctr_dict[cls].copy()
            cor_pattern = os.path.join(
                self.config.kp_orbfps_dir, '{}_corners.txt'.format(cls),
            )
            cors = np.loadtxt(cor_pattern.format(cls), dtype=np.float32)
            ctr = cors.mean(0)
            self.custom_cls_ctr_dict[cls] = ctr
        return ctr.copy()
