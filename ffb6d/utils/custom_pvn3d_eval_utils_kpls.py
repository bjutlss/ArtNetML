...
config = Config(ds_name='custom')
bs_utils = Basic_Utils(config)
cls_lst = config.custom_cls_lst
try:
    config_custom = Config(ds_name="custom")
    bs_utils_custom = Basic_Utils(config_custom)
...
def cal_frame_poses_custom(pcld, mask, ctr_of, pred_kp_of, use_ctr, n_cls, use_ctr_clus_flter, obj_id, debug=True):
    """
    Calculates pose parameters by 3D keypoints & center points voting to build
    the 3D-3D corresponding then use least-squares fitting to get the pose parameters.
    """
    ...
    if cls_msk.sum() < 1:
        ...
        # visualize
        if debug:
            show_kp_img = np.zeros((1920, 1440, 3), np.uint8)
            kp_2ds = bs_utils.project_p3d(
                cls_kps[cls_id].cpu().numpy(), 1000.0, K='custom'
            )
            print("cls_id = ", cls_id)
        ...
        mesh_kps = bs_utils_custom.get_kps(obj_id, ds_type="custom")
        if use_ctr:
            mesh_ctr = bs_utils_custom.get_ctr(obj_id, ds_type="custom").reshape(1, 3)
            mesh_kps = np.concatenate((mesh_kps, mesh_ctr), axis=0)
...
def eval_metric_custom\
    ...
    pred_RT = torch.from_numpy(pred_RT.astype(np.float32)).cuda()
    gt_RT = RTs[0]
    mesh_pts = bs_utils_custom.get_pointxyz_cuda(obj_id, ds_type="custom").clone()
    add = bs_utils_custom.cal_add_cuda(pred_RT, gt_RT, mesh_pts)
    adds = bs_utils_custom.cal_adds_cuda(pred_RT, gt_RT, mesh_pts)
...
def eval_one_frame_pose_custom(item):
    pcld, mask, ctr_of, pred_kp_of, RTs, cls_ids, use_ctr, n_cls, \
    ...
    cls_add_dis, cls_adds_dis = eval_metric_custom(
        cls_ids, pred_pose_lst, RTs, mask, label, obj_id
    )
    return (cls_add_dis, cls_adds_dis)
...
class TorchEval():
    ...
    def cal_custom_add(self, obj_id, test_occ=False):
        add_auc_lst = []
        adds_auc_lst = []
        add_s_auc_lst = []
        cls_id = obj_id
        if (obj_id) in config_custom.custom_sym_cls_ids:
            self.cls_add_s_dis[cls_id] = self.cls_adds_dis[cls_id]
        else:
            self.cls_add_s_dis[cls_id] = self.cls_add_dis[cls_id]
        self.cls_add_s_dis[0] += self.cls_add_s_dis[cls_id]
        add_auc = bs_utils_custom.cal_auc(self.cls_add_dis[cls_id])
        adds_auc = bs_utils_custom.cal_auc(self.cls_adds_dis[cls_id])
        add_s_auc = bs_utils_custom.cal_auc(self.cls_add_s_dis[cls_id])
        add_auc_lst.append(add_auc)
        adds_auc_lst.append(adds_auc)
        add_s_auc_lst.append(add_s_auc)
        d = config_custom.custom_r_lst[obj_id]['diameter'] / 1000.0 * 0.1
        ...
        cls_type = config_custom.custom_id2obj_dict[obj_id]
    ...
    def eval_pose_parallel(
        ...
        with concurrent.futures.ThreadPoolExecutor(
            ...
            eval_func = eval_one_frame_pose_custom
